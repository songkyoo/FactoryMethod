    ...
    private static ImmutableArray<string> GenerateFactoryMethodCode(INamedTypeSymbol typeSymbol)
    {
        ...
        static IEnumerable<IMethodSymbol> FilterTargetConstructors(INamedTypeSymbol typeSymbol, bool hasTypeAttribute)
        {
            return typeSymbol
                ...
                .Where(methodSymbol =>
                {
                    if (HasIgnoreFactoryMethodAttribute(methodSymbol))
                    {
                        return false;
                    }
                    ...
                    if (hasTypeAttribute)
                    {
                        return
                            ...
                            (declaredAccessibility is Private && HasGenerateFactoryMethodAttribute(methodSymbol));
                    }
                    else
                    {
                        return
                            declaredAccessibility is Public or Internal &&
                            HasGenerateFactoryMethodAttribute(methodSymbol);
                    }
                });
        }
        #endregion
